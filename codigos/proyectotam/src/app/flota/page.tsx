...
import { getVehicles, addVehicle, updateVehicle, deleteVehicle } from "@/lib/actions/plantaActions"
import { useAuthStore } from "@/lib/store/auth"
import { ShieldCheck, Plus, Trash2, Shield, Search } from "lucide-react"
...
export default function FlotaPage() {
    ...
    const handleAddVehicle = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const ni = fd.get("ni") as string
        const origen_unit = fd.get("origen_unit") as string

        const res = await addVehicle({ ni, origen_unit, status: 'in_army', army_status: 'uninspected' })
        if (res.success) {
            loadData()
                                ...
                                <table className="w-full text-sm text-left">
                                    ...
                                    <tbody className="divide-y">
                                        ...
                                        {filteredVehicles.length === 0 && (
                                            <tr><td colSpan={4} className="px-6 py-8 text-center text-slate-500">No hay vehículos en el ejército con ese filtro.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            )}
                    ...
                    <Card>
                        ...
                        <CardContent className="pt-4">
                            <form onSubmit={handleAddVehicle} className="space-y-4">
                                <div>
                                    <label className="text-xs font-semibold text-slate-700">Número de Identificación (NI)</label>
                                    <Input type="text" name="ni" required className="mt-1" placeholder="Ej: 410332" />
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-slate-700">Unidad Geográfica / Origen</label>
                                    <Input type="text" name="origen_unit" required className="mt-1" placeholder="Ej: RC Tan 8" />
                                </div>
                                <Button type="submit" className="w-full bg-green-600 hover:bg-green-700">Añadir al Listado</Button>
                            </form>
                        </CardContent>
                    </Card>
                </div>
