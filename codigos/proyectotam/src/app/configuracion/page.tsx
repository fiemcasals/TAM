...
import { Settings, Plus, Edit2, Trash2, Save, X, ListChecks } from "lucide-react"
                            ...
                            <div className="bg-slate-100 p-4 border-b flex justify-between items-center">
                                {editingAct === act.id ? (
                                    <div className="flex items-center gap-3 w-full max-w-xl">
                                        <Input value={actOrder} type="number" onChange={e => setActOrder(parseInt(e.target.value) || 0)} className="w-20" />
                                        <Input value={actName} onChange={e => setActName(e.target.value)} className="flex-1" />
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="bg-slate-200 text-slate-700 font-bold px-3 py-1 rounded text-sm">Paso {act.suggested_order}</div>
                                        ...
                                        <div key={check.id} className="flex justify-between items-center bg-slate-50 p-3 rounded border border-slate-100 group">
                                            {editingCheck === check.id ? (
                                                <div className="flex items-center gap-3 w-full">
                                                    <Input value={checkDesc} onChange={e => setCheckDesc(e.target.value)} className="flex-1 h-8 text-sm" />
